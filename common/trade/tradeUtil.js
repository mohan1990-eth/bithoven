...
class TradeUtil {
   ...
   */
  static async getLargestKeyFleetOwnerOfGamer(
    gamerAddress,
    portfolioName = "default"
  ) {
    const keyFleet = new KeyFleet();
    const holderAddresses = keyFleet.getAllAddresses(portfolioName);
    let largestOwnerAddress = null;
    let maxBalance = ethers.BigNumber.from(0);

    for (const holderAddress of holderAddresses) {
      const bitBalance = ethers.BigNumber.from(
        await this.getBitBalanceInStore(holderAddress, gamerAddress)
      );
  ...
  static async getProposedSum(gamerAddress, tradeType, holderAddress = null) {
    const proposedOrdersDir = path.join(
      __dirname,
      `../../data/orders/proposedOrders/${tradeType.toLowerCase()}`,
      gamerAddress
    );
    ...
    const orderFiles = (await fs.readdir(proposedOrdersDir)).filter((file) =>
      file.endsWith(".json")
    );
    let totalQuantity = 0;
  ...
  static async getAmountPending(gamerAddress, holderAddress, tradeType) {
    ...
    if (
      pendingOrder.gamerAddress === gamerAddress &&
      pendingOrder.orderType === tradeType
    ) {
   ...
   */
  static async adjustSellTargetAmount(
    gamerAddress,
    holderAddress,
    maxBitsToSell
  ) {
    const bitBalance = await this.getBitBalanceInStore(
      holderAddress,
      gamerAddress
    );
    const amountPending = await this.getAmountPending(
      gamerAddress,
      holderAddress,
      "SELL"
    );
    const proposedSum = await this.getProposedSum(
      gamerAddress,
      "SELL",
      holderAddress
    );
  ...
  static async computePandL(store, startStrategyBlockNumber) {
      ...
      for (const gamerAddress in batches) {
        ...
        for (const batchFile in batchFiles) {
          ...
          if (
            ...
            batch.BlockNumOnWhichBitsWereBought < startStrategyBlockNumber
          ) {
            continue;
          }
          ...
          const remainingBatchAmount = new BigNumber(
          ...
          const adjustedInitialInvestment = purchasePrice
            .dividedBy(initialBatchAmount)
            .multipliedBy(sellAmount);
          const profitDelta = sellPrice.minus(adjustedInitialInvestment);

          cumulativeProfitDelta = cumulativeProfitDelta.plus(profitDelta);
          totalAdjustedInitialInvestment = totalAdjustedInitialInvestment.plus(
    ...
    for (const holderAddress in store.holders) {
      ...
      const totalProfitDelta = holderResult.cumulativeProfitDelta.plus(
        archiveResult.cumulativeProfitDelta
      );
      const totalInitialInvestment =
        holderResult.totalAdjustedInitialInvestment.plus(
          archiveResult.totalAdjustedInitialInvestment
        );

      const absoluteProfitEth = totalProfitDelta
        .dividedBy(new BigNumber(10).pow(18))
        .toFixed(4);
      const percentProfit = totalInitialInvestment.isZero()
        ? "0"
        : totalProfitDelta
            .dividedBy(totalInitialInvestment)
            .multipliedBy(100)
            .toFixed(4);
      const adjustedInitialInvestmentEth = totalInitialInvestment
        .dividedBy(new BigNumber(10).pow(18))
        .toFixed(4);
      ...
      result.total.absoluteProfit = result.total.absoluteProfit.plus(
        totalProfitDelta.dividedBy(new BigNumber(10).pow(18))
      );
      result.total.totalAdjustedInitialInvestment =
        result.total.totalAdjustedInitialInvestment.plus(
          totalInitialInvestment
        );
    }

    const totalPercentProfit =
      result.total.totalAdjustedInitialInvestment.isZero()
        ? "0"
        : result.total.absoluteProfit
            .dividedBy(
              result.total.totalAdjustedInitialInvestment.dividedBy(
