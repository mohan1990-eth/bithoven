    ...
    .map((rule) => {
      ...
      if (rule.conditions) {
        conditions = rule.conditions.map((condition) => {
          const { functionName, args } = parseAndValidateExpression(
      ...
      const { functionName: actionFunctionName, args: actionArgs } =
        parseAndValidateExpression(rule.action, functions);
      return {
        conditions,
        quantity,
        action: { functionName: actionFunctionName, args: actionArgs },
        ruleID: rule.ruleID,
      };
    });
...
async function evaluateAndInvokeRulesAsync(ctx, rules) {
  ...
  for (const rule of rules) {
      ...
      allConditionsMet = (
        await Promise.all(
          rule.conditions.map(async (condition) => {
            return await functions[condition.functionName](
              ctx,
              ...condition.args
            );
          })
    ...
    if (allConditionsMet && rule.quantity) {
      ctx.quantity = await functions[rule.quantity.functionName](
        ctx,
        ...rule.quantity.args
      );
    }
