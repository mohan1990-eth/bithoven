...
async function main() {
  ...
  const newGamerFeedInstance = new CloudNewGamerFeed(
  ...
  indexer.fetchEventsPeriodically(() => {
    if (!indexerCaughtUp) {
      console.log(
        "--- [Investor] calling newGamerFeedInstance.startProcessingNewUsers"
      );
      newGamerFeedInstance.startProcessingNewUsers();
      console.log(
        "--- [Investor] calling chainHolderInvestmentsInstance.startFullSweep"
      );
      chainHolderInvestmentsInstance.startFullSweep();
      console.log("--- [Investor] calling copyTrader.startCopyTrader");
      copyTrader.startCopyTrader();
...
main().catch((error) => {
  logger.logError(
    { msg: `Error starting ChainIndexer: ${error.message}` },
    "INVESTOR_ERROR"
  );
