...
const {
  ...
  processJsonSellRules,
  evaluateAndInvokeSell,
} = require("../../common/rulesEngineLib");

const {
  readAllPortfolios,
  findPortfolioByKeyFleetAddress,
...
class ChainIndexer {
  constructor() {
    ...
    this.buyRulesFilePath = path.resolve(
      __dirname,
      "../../rules/buy/buyRules.json"
    );
    this.sellRulesFilePath = path.resolve(
      __dirname,
      "../../rules/sell/sellRules.json"
    );
  ...
  processBuyRules() {
    const jsonDocument = JSON.parse(
      fs.readFileSync(this.buyRulesFilePath, "utf-8")
    );
  ...
  processSellRules() {
    const jsonDocument = JSON.parse(
      fs.readFileSync(this.sellRulesFilePath, "utf-8")
    );
  ...
  async storeEventDetails(eventDetails) {
    ...
    if (bitAmount === 0) {
      console.log(
        `[ChainIndexer] Skipping batch for gamer ${gamerAddress} under holder ${holderAddress} due to bitAmount being zero.`
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        `[ChainIndexer] Failed to add holder ${holderAddress}:`,
        error
      );
      throw error;
    }
    ...
    if (eventDetails.isBuy) {
      ...
      try {
        ...
        console.log(
          `[ChainIndexer] Added new batch for gamer ${gamerAddress} under holder ${holderAddress}`
        );
      } catch (error) {
        if (error instanceof JSONStoreError) {
          console.error(
            `[ChainIndexer] Add Gamer Batch Error - Code: ${error.code}, Message: ${error.message}`
          );
          throw error;
        } else {
      ...
      const sellPrice = ethers.BigNumber.from(
        eventDetails.ethAmount
      ).toString();

      try {
        const batchFiles = await this.store.getBatchFilesInAscendingOrder(
          holderAddress,
          gamerAddress
        );
        for (const batchFile of batchFiles) {
          ...
          const batch = await this.store.getBatchFile(
            holderAddress,
            gamerAddress,
            batchNumber
          );
          ...
          if (batch.remainingBatchAmount >= bitsToDeduct) {
            await this.store.updateGamerBatch(
              holderAddress,
              gamerAddress,
              batchNumber,
              bitsToDeduct,
            ...
            console.log(
              `Updated batch ${batchNumber} for gamer ${gamerAddress} under holder ${holderAddress}`
            );
            bitsToDeduct = 0;
            break;
          } else {
            ...
            await this.store.updateGamerBatch(
              ...
              gamerAddress,
              batchNumber,
              batch.remainingBatchAmount,
              blockNumber,
              sellPrice
            );
            console.log(
              `[ChainIndexer] Updated batch ${batchNumber} for gamer ${gamerAddress} under holder ${holderAddress}, bits remaining to deduct: ${bitsToDeduct}`
            );
          }
        }
        ...
        if (bitsToDeduct > 0) {
          console.error(
            `[ChainIndexer] Not enough bits to deduct for gamer ${gamerAddress} under holder ${holderAddress}`
          );
        ...
        if (error instanceof JSONStoreError) {
          console.error(
            `[ChainIndexer] Update Gamer Batch Error - Code: ${error.code}, Message: ${error.message}`
          );
          throw error;
        } else {
          console.error(`[ChainIndexer] Unexpected Error: ${error.message}`);
    ...
    // Read latest portfolios from portfolioConfig.json file in the config folder
    this.allPortfolios = readAllPortfolios();

    const holderPortfolio = findPortfolioByKeyFleetAddress(
      this.allPortfolios,
      holderAddress
    );

    // if holder portfolio is not null, then the holder address is a key fleet address of one of the portfolios
    console.log("[ChainIndexer] Holder Portfolio: ", holderPortfolio);
    if (holderPortfolio != null) {
    ...
    if (this.indexingCaughtUp) {
      ...
      const portfolioOfCopiedAddress = isHolderAddressACopiedTraderAddress(
        this.allPortfolios,
        holderAddress
      );
      // if the holder address is a copied trader address, copyBuy and copySell actions will be executed
      // copyBuyImpl() and copySellImpl() in comm/actions/actions.js expects a portfolio object in the context object
      if (portfolioOfCopiedAddress != null) {
        ctx["portfolio"] = portfolioOfCopiedAddress;
      }

      if (this.buyRules) {
        console.log(
          "[ChainIndexer] calling shimCopyTradeActionForQuantity for buyRules"
        );
        this.buyRules = CopyTrader.shimCopyTradeActionForQuantity(
          this.buyRules,
          eventDetails.bitAmount
        );

        // console.log("this.buyRules: " + JSON.stringify(this.buyRules));
      ...
      if (this.sellRules) {
        console.log(
          "[ChainIndexer] calling shimCopyTradeActionForQuantity for sellRules"
        );
        this.sellRules = CopyTrader.shimCopyTradeActionForQuantity(
          this.sellRules,
          eventDetails.bitAmount
        );

        // console.log("this.sellRules: " + JSON.stringify(this.sellRules));
        // console.log("ctx: " + JSON.stringify(ctx));
  ...
  async fetchEvents(startBlock, endBlock, batchSize) {
    for (let i = startBlock; i <= endBlock; i += batchSize) {
      ...
      console.log(
        `[ChainIndexer] Fetching events from block ${fromBlock} to ${toBlock}`
      );
      try {
        const eventLogs = await this.contract.queryFilter(
          this.contract.filters.Trade(),
          fromBlock,
          toBlock
        );
        ...
        console.error(
          `[ChainIndexer] Error fetching events from block ${fromBlock} to ${toBlock}:`,
          error
        );
        throw error;
      }
  ...
  async catchupIndex() {
    try {
      ...
      if (latestIndexedBlockNum > 0) {
        ...
        console.log(
          `[ChainIndexer] Starting from last indexed Block: ${startBlock} (contract deployed at block ${config.startBlock}) Network: ${config.network}`
        );
      } else {
        console.log(
          `[ChainIndexer] Starting from contract deployment block ${config.startBlock}) Network: ${config.network}`
        );
      }
      ...
      logger.logError(
        {
          msg: `[ChainIndexer] Error determining start block: ${error.message}`,
        },
        "CHAIN_INDEXER_ERROR"
      );
      throw error;
    ...
    const fetch = async () => {
      try {
        ...
        if (startBlock <= endBlock) {
          ...
          console.log(
            "[ChainIndexer] No new blocks to process, waiting for new blocks..."
          );
          this.indexingCaughtUp = true;
          console.log("[ChainIndexer] Process Rules Green lit");
        }
        ...
        console.error(
          "[ChainIndexer] Error fetching events in periodic call:",
          error
        );
        logger.logError(
          {
            msg: `[ChainIndexer] Error fetching events from the blockchain: ${error.message}. Investor.js will restart in ${config.restartDelaySeconds} seconds If you have it under pm2 control. It will then automatically start indexing from the last successfuly saved event block.`,
          },
          "CHAIN_INDEXER_ERROR"
        );
        console.log(
          `[ChainIndexer] Restarting investor.js in ${config.restartDelaySeconds} seconds...`
        );
        await new Promise((resolve) =>
          setTimeout(resolve, config.restartDelaySeconds * 1000)
        );
